#!/bin/sh
# install.sh - RVR L2 Bridge installer
...
INSTALL_DIR="/root/RVR_v0.3"
MIN_DISK_MB=40

# Package lists (must match download-packages.sh originals)
COMMON_PKGS="tinc kcptun-config kmod-tun libopenssl3 liblzo2 zlib git git-http libcurl4"
...
# HTTP FETCH HELPER
#############################################
# Finds wget or uclient-fetch (BusyBox default on OpenWrt)
fetch() {
...
ensure_opkg_config() {
    ...
    # Architecture entries for GL.iNet and standard OpenWrt
    ...
    # GL.iNet firmware: add official OpenWrt packages feed
    local arch
...
download_repo() {
    ...
    info "Downloading RVR L2 Bridge..."
    ...
    if ! fetch "$TARBALL_URL" "$tmp_tar" 2>/dev/null; then
        ...
        if [ -n "$token" ]; then
            ...
            # Save token for git remote later
...
install_packages() {
    ...
    case "$role" in
        gcs)
            ...
            if command -v tinc >/dev/null 2>&1 && command -v kcptun-server >/dev/null 2>&1; then
                ok "GCS packages installed"
            else
                fail "Package verification failed (tinc or kcptun-server missing)"
            fi
            ;;
        aircraft)
            ...
            if command -v tinc >/dev/null 2>&1 && command -v kcptun-client >/dev/null 2>&1; then
                ok "Aircraft packages installed"
            else
                fail "Package verification failed (tinc or kcptun-client missing)"
            fi
            ;;
    esac
}
...
#############################################
# REFRESH PACKAGES (replaces download-packages.sh)
#############################################
refresh_packages() {
    info "Refreshing bundled packages from opkg feeds..."
    info "This downloads fresh .ipk files for offline installation on other devices."

    local pkg_dir="$INSTALL_DIR/packages"

    # Need opkg update for this (slow but necessary for refresh)
    info "Updating package feeds (this may take ~30 seconds)..."
    opkg update >/dev/null 2>&1 || { warn "opkg update failed"; return 1; }

    # Clear and recreate package directories
...
EOF
    ...
    echo "    common/:   $common_count packages"
...
# Kernel check (needs packages/manifest.txt from repo)
check_kernel_version
...
# Interactive: bootstrap git before menu (needed for l2bridge symlink)
# Packages will be installed via menu role selection
bootstrap_git
setup_git_repo
install_l2bridge
